...
def main():
    ...
    # Declare vae policy
    vape = VAEPolicy(avoidance='self')
    if args.policy:
        vape.load_state_dict(torch.load(args.policy))
    # Declare sampler
    ...
    for i_episode in trange(0, args.episodes, args.eb, desc="Episodes", unit_scale=args.eb):
        # Sample trajectories
        trajectories, losses_and_info = sampler.sample(args.eb, vape, render=(i_episode%args.render_interval==0))
